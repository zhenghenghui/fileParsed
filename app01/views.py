...
def getFile(request):
    if request.method == 'POST':
        ...
        print('name:',request.FILES.get('file').name)
        print(type(request.FILES.get('file')))
        localPath = './media'
        if not os.path.exists(localPath):
            os.mkdir(localPath)
        file_path = os.path.join(localPath, audioFile.name)
        ...
        time1 = datetime.now()
        global wsParam
        wsParam = Ws_Param(APPID='bbeaa091', APISecret='NWI3ZmNiYzQ0NTNhNzMzMDJlOTgyMTk3',
                           APIKey='72ee5be208d4daafba171e7272f5fa5e',
                           AudioFile=ouputFilePath)
